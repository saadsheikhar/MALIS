...
A = Game(1,1,1,1,1,1)
# Code to create  empty black window of 800 x 600 resolution
...
score, lines = 0, 0
scores = {0: 0, 1: 100, 2: 300, 3: 700, 4: 1500}  # Score and Bonus points depending on the number of lines destroyed
...
while True:
    ...
    for i in range(4):  # 4 blocks per figure
        figure_rect.x = figure[i].x * A.TILE
        figure_rect.y = figure[i].y * A.TILE
        pygame.draw.rect(game_screen, color, figure_rect)

    for y, raw in enumerate(field):
        for x, col in enumerate(raw):
            if col:
                figure_rect.x, figure_rect.y = x * A.TILE, y * A.TILE
                pygame.draw.rect(game_screen, col, figure_rect)

    # Draw Next figure

    for i in range(4):  # 4 blocks per figure
        figure_rect.x = next_figure[i].x * A.TILE + 450  # Overflow x to render on right
        figure_rect.y = next_figure[i].y * A.TILE + 250  # Overflow y
    ...
    for i in range(A.W):
        if field[0][i]:
            ...
            field = [[0 for i in range(A.W)] for i in range(A.H)]  # Clean game map
